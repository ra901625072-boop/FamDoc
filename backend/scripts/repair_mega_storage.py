...
def repair():
    ...
    try:
        ...
        mega_config = family_config.get("mega", {})
        
        provider = get_storage_provider("mega")
        client = provider._get_client(mega_config)
        
        # Fetch MEGA files list EXACTLY ONCE to avoid rate limits
        print("Fetching MEGA files list...")
        files_in_mega = client.get_files()
        print(f"Total items found in MEGA: {len(files_in_mega)}")
        
        # 1. Verify/Ensure Vault Folder exists in files_in_mega
        ...
        if vault_folder_id not in files_in_mega:
            ...
            vault_folder_id = provider.ensure_vault_folder(family.id, mega_config, db=db)
        else:
